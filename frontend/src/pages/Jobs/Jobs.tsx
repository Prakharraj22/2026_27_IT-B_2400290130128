...
export function Jobs() {
  ...
  const [noMatchesHint, setNoMatchesHint] = useState<string | null>(null);
  ...
  useEffect(() => {
    getRecommendedJobs().then((result) => {
      setJobs(result);
      setNoMatchesHint(getRecommendedJobsHint());
    });
  }, []);

  const noFiltersApplied = !query && !remoteOnly && !activeType;
  ...
  return (
    <div>
      ...
      ) : filtered.length === 0 && noFiltersApplied && noMatchesHint ? (
        <EmptyState
          icon={<Briefcase className="h-5 w-5" />}
          title="No matches yet"
          description={noMatchesHint}
        />
      ) : filtered.length === 0 ? (
        <EmptyState
          icon={<Briefcase className="h-5 w-5" />}
