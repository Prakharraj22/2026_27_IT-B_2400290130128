...
## 12. Projects/Experience/Certifications on Profile

- **Decision:** Added `projects`, `experience`, and `certifications` as JSONB array columns directly on `auth.profiles` (migration `20260918000000_add_profile_projects_experience_certifications`), validated via nested DTOs (`ProjectDto`/`ExperienceDto`/`CertificationDto` in `src/profiles/dto/profile-items.dto.ts`) and writable through the existing `PATCH /v1/profiles/me` endpoint.
- **Reason:** These were previously frontend-only mock data with no backend field at all — "Add Project"/"Add Experience"/"Add Certification" buttons existed in the UI but did nothing. Unlike `skills`, this data has no other owner (no Resume Module claim on it), so it's safe and correct for the Auth & Profiles module to own outright. The same JSONB-array-on-Profile pattern already used for `skills` was reused rather than introducing separate relational tables — there's no cross-cutting query need (nothing joins against individual projects/experience/certification rows), so normalized tables would be premature complexity for what's fundamentally a simple, per-user, read-modify-write list.
- **Trade-offs:** Not independently queryable/indexable (e.g., "find all users with an AWS certification" isn't an efficient query with this shape). If that becomes a real requirement, promote to real tables with foreign keys to `Profile`.
- **How to Change Later:** Add `Project`/`Experience`/`Certification` models with a `profileId` foreign key, migrate the JSONB data across, and add dedicated CRUD endpoints/repository methods in place of the current whole-array PATCH.

## 13. External Module Contracts (Resume, AI Worker, Notifications)
